    ...
    def initialize_firebase(self):
        ...
        try:
            if init_firebase_with_credentials():
                ...
                # Store initial logs when Firebase becomes available during initialization
                if self.session_logs:
                    self.log_queue.put(("Firebase", f"💾 Saving {len(self.session_logs)} startup log entries to Firebase..."))
                    if store_session_log(self.session_logs):
                        self.log_queue.put(("Firebase", "✅ Startup logs saved successfully"))
                        # Clear logs after successful save to prevent duplicate saves
                        self.session_logs = []
                    else:
                        self.log_queue.put(("Firebase", "❌ Failed to save startup logs"))
            else:
                self.log_queue.put(("Firebase", "⚠️ Firebase connection failed. Logs will not be saved."))
        except Exception as e:
            # Catch double initialization errors and just report success
            if "already exists" in str(e):
                self.log_queue.put(("Firebase", "✅ Firebase already initialized."))
                # Store initial logs when Firebase was already available
                if self.session_logs:
                    self.log_queue.put(("Firebase", f"💾 Saving {len(self.session_logs)} startup log entries to Firebase..."))
                    if store_session_log(self.session_logs):
                        self.log_queue.put(("Firebase", "✅ Startup logs saved successfully"))
                ...
                self.log_queue.put(("Firebase", f"⚠️ Firebase initialization error: {e}"))
